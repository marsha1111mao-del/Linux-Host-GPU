...
static const struct test_hvc_info hvc_info[] = {
	...
	TEST_HVC_INFO(ARM_SMCCC_VENDOR_HYP_GPA_TO_HPA_FUNC_ID, 0),
};
...
static void guest_test_hvc(const struct test_hvc_info *hc_info)
{
	...
	hvc_info_arr_sz = hc_info == hvc_info ? ARRAY_SIZE(hvc_info) :
						ARRAY_SIZE(false_hvc_info);

	for (i = 0; i < hvc_info_arr_sz; i++, hc_info++) {
		memset(&res, 0, sizeof(res));
		smccc_hvc(hc_info->func_id, hc_info->arg1, 0, 0, 0, 0, 0, 0,
			  &res);
...
static void steal_time_init(struct kvm_vcpu *vcpu)
{
	...
	vm_userspace_mem_region_add(vcpu->vm, VM_MEM_SRC_ANONYMOUS, ST_GPA_BASE,
				    1, gpages, 0);
...
static void test_fw_regs_before_vm_start(struct kvm_vcpu *vcpu)
{
	...
	for (i = 0; i < ARRAY_SIZE(fw_reg_info); i++) {
		...
		TEST_ASSERT(
			val == FW_REG_ULIMIT_VAL(reg_info->max_feat_bit),
			"Expected all the features to be set for reg: 0x%lx; expected: 0x%lx; read: 0x%lx",
			reg_info->reg,
			FW_REG_ULIMIT_VAL(reg_info->max_feat_bit), val);
		...
		if (reg_info->max_feat_bit < 63) {
			ret = __vcpu_set_reg(vcpu, reg_info->reg,
					     BIT(reg_info->max_feat_bit + 1));
			TEST_ASSERT(
				ret != 0 && errno == EINVAL,
...
static void test_fw_regs_after_vm_start(struct kvm_vcpu *vcpu)
{
	...
	for (i = 0; i < ARRAY_SIZE(fw_reg_info); i++) {
		 ...
		 */
		ret = __vcpu_set_reg(vcpu, reg_info->reg,
				     FW_REG_ULIMIT_VAL(reg_info->max_feat_bit));
		TEST_ASSERT(
			ret != 0 && errno == EBUSY,
